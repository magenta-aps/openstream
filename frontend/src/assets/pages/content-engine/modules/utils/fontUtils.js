...
export async function fetchAndInitializeFonts() {
  ...
  if (fontInitializationPromise) {
    console.log("Font initialization already in progress, waiting for completion...");
...
async function _doFetchAndInitializeFonts() {
  try {
    ...
    if (fonts && Array.isArray(fonts)) {
      console.log(`Successfully fetched ${fonts.length} fonts:`, fonts.map(f => f.name));
      ...
      console.log("Using @font-face stylesheet injection (forced for slideshow-player compatibility)...");
      ...
      */
      ...
      console.log(`Font initialization complete. Available fonts: ${availableFonts.length}`);
      return fonts;
    } else {
...
function injectFontFacesIntoStylesheet(fonts) {
  ...
  console.log(`@font-face injection complete: ${successCount}/${fonts.length} rules added`);
}
...
async function loadProtectedFontsViaFontFace(fonts, headers = {}) {
  if (!window.FontFace) {
    ...
    console.log("FontFace API not supported, falling back to stylesheet injection");
  ...
  const fontLoadPromises = fonts.map(async (font) => {
    ...
    try {
      ...
      console.log(`Font ${font.name} fetched, size: ${arrayBuffer.byteLength} bytes`);
      ...
      const isInDocumentFonts = Array.from(document.fonts.values()).some(f => f.family === font.name);
      console.log(`Font ${font.name} in document.fonts: ${isInDocumentFonts}`);
      
      console.log(`Font ${font.name} successfully loaded and added to document.fonts`);
      console.log(`FontFace status: ${fontFace.status}, loaded: ${fontFace.loaded}`);
      
      // Verify it was actually added
      const fontsInDocument = Array.from(document.fonts.values()).map(f => f.family);
      console.log(`Fonts now in document.fonts: [${fontsInDocument.join(', ')}]`);
  ...
  console.log(`Font loading complete: ${successful}/${fonts.length} fonts loaded successfully`);
  ...
  if (failed > 0) {
    console.warn(`${failed} fonts failed to load via FontFace API. Failed fonts:`);
...
async function verifyFontsLoaded(fonts) {
  ...
  for (const font of fonts) {
    ...
    try {
      ...
      console.log(`Font "${font.name}" availability check: ${isAvailable ? 'AVAILABLE' : 'NOT AVAILABLE'}`);
      
      if (!isAvailable) {
        console.warn(`Font "${font.name}" is not available despite loading attempt`);
      }
    } catch (e) {
...
async function verifyFontsActuallyWork() {
  ...
  try {
    ...
    for (const font of availableFonts) {
      ...
      console.log(`Font "${font.name}" rendering test: width diff = ${widthDiff.toFixed(2)}px`);
      
      // Lower threshold since SUSEMono shows 0.45px difference but works
      if (widthDiff > 0.4) {
        console.log(`✓ Font "${font.name}" appears to be rendering correctly (diff: ${widthDiff.toFixed(2)}px)`);
      } else {
        console.warn(`⚠ Font "${font.name}" may not be rendering (width diff: ${widthDiff}px)`);
        return false;
      }
    }
...
export async function waitForFontsReady(maxWaitMs = 3000) {
  ...
  console.log(`Waiting for ${availableFonts.length} fonts to be ready (max ${maxWaitMs}ms)...`);
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    for (const font of availableFonts) {
      ...
      try {
        ...
        currentResults.push({ name: font.name, ready: false, error: e.message });
        allReady = false;
      }
    }
    
    // Log detailed results on first check or when results change
    if (lastCheckResults.length === 0 || JSON.stringify(currentResults) !== JSON.stringify(lastCheckResults)) {
      console.log(`Font readiness check at ${Date.now() - startTime}ms:`, currentResults);
    ...
    if (allReady) {
      ...
      if (verification) {
        ...
        console.warn("⚠ Font rendering verification failed, waiting longer...");
      }
    }
  ...
  const waitTime = Date.now() - startTime;
  console.warn(`⚠ Font readiness timeout after ${waitTime}ms - proceeding anyway`);
...
export function debugFonts() {
  ...
  if (document.fonts && document.fonts.values) {
    ...
    console.log("Fonts in document.fonts:", loadedFonts.map(f => ({
      family: f.family,
      status: f.status,
      loaded: f.loaded
    })));
  ...
  if (customStylesheet && customStylesheet.sheet) {
    console.log("@font-face rules in stylesheet:", 
  ...
  availableFonts.forEach(font => {
    if (font.name) {
      try {
        const isAvailable = document.fonts.check(`12px "${font.name}"`);
        console.log(`Font "${font.name}" check:`, isAvailable ? "✓ Available" : "✗ Not available");
      } catch (e) {
...
if (typeof window !== 'undefined') {
  ...
  window.checkPageFonts = function() {
    ...
    const slideElements = document.querySelectorAll('.textbox .text-content, .table-element, .list-element');
    console.log(`Found ${slideElements.length} slide elements with potential custom fonts:`);
    
    slideElements.forEach((el, index) => {
      const computed = window.getComputedStyle(el);
      const innerHTML = el.innerHTML ? el.innerHTML.substring(0, 50) + '...' : 'No content';
    ...
    const textboxSpans = document.querySelectorAll('.textbox .text-content span[data-font-family]');
    if (textboxSpans.length > 0) {
      console.log(`Found ${textboxSpans.length} spans with data-font-family attributes:`);
  ...
  window.forceApplyFont = function(fontName = null) {
    const targetFont = fontName || (availableFonts.length > 0 ? availableFonts[0].name : null);
    if (!targetFont) {
    ...
    textboxes.forEach((textbox, index) => {
      ...
      console.log(`Textbox ${index + 1}: Changed from "${originalFont}" to "${textbox.style.fontFamily}"`);
    ...
    console.log(`Force-applied "${targetFont}" to slide elements. Check visually if font changed.`);
  };
}
